...
const defaultTitles = {
  en: {note: 'Note', tip: 'Tip', info: 'Info', warning: 'Warning', danger: 'Danger', design: 'Design note'},
  fr: {note: 'Note', tip: 'Conseil', info: 'Information', warning: 'Attention', danger: 'Danger', design: 'Note de design'},
};
const publicationLabels = {
  en: {version: 'Version', revision: 'Revision', designedWith: 'designed with', poweredBy: 'powered by'},
  fr: {version: 'Version', revision: 'Révision', designedWith: 'conçu avec', poweredBy: 'propulsé par'},
};
...
function linkHtml(link) {
  ...
  return link.href ? `<a href="${escapeHtml(link.href)}">${label}</a>` : label;
}
...
function transformAdmonitions(markdown, locale, customTypes) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (fence) {
      ...
      else if (fence[1][0] === fenceMarker) { inFence = false; fenceMarker = null; }
      output.push(line);
      continue;
    }
    ...
    if (!allowed.has(opening.type)) {
      throw new Error(`Unsupported Markdown directive :::${opening.type}. Declare it in publications.config.mjs before publishing it.`);
    }
    const body = [];
    let foundClosing = false;
    for (index += 1; index < lines.length; index += 1) {
      if (lines[index].trim() === ':::') { foundClosing = true; break; }
      if (parseOpening(lines[index])) throw new Error('Nested admonitions are not supported by the publication builder yet.');
      body.push(lines[index]);
    }
    if (!foundClosing) throw new Error(`Unclosed :::${opening.type} admonition.`);
    const fallbackTitle = defaultTitles[locale]?.[opening.type] ?? opening.type.replaceAll('-', ' ');
    const title = opening.title || fallbackTitle;
    const marker = `<span class="publication-admonition-title publication-admonition-${opening.type}">${escapeHtml(title)}</span>`;
    output.push(`> ${marker}`);
    output.push('>');
    for (const bodyLine of body) output.push(bodyLine.length === 0 ? '>' : `> ${bodyLine}`);
  }
  return `${output.join('\n')}\n`;
}
...
function outputTargets(baseName, locale, formats) {
  return formats.map((format) => ({path: path.join(outputRoot, assetName(baseName, locale, format)), format}));
}

function publicationManifest(version) {
  const paths = new Set();
  const publications = Object.entries(config.publications).map(([publicationName, publication]) => {
    ...
    for (const [locale, localeConfig] of Object.entries(publication.locales)) {
      const formats = localeConfig.outputs.map((format) => {
        ...
        if (paths.has(collisionKey)) throw new Error(`Publication output collision: ${collisionKey}`);
        paths.add(collisionKey);
        return {format, path: assetPath};
      });
      locales[locale] = {title: localeConfig.title, formats};
    }
    return {id: publicationName, outputName: baseName, revision: publication.revision ?? null, locales};
  });
  return {version, publications};
}

function normalizeCover(cover) {
  if (!cover) return null;
  if (typeof cover === 'string') return {image: cover, showTitle: true, showMetadata: true};
  return {showTitle: true, showMetadata: true, ...cover};
}
...
function licenseHtml(license) {
  ...
  if (!attribution) return licenseLink;
  const title = attribution.href ? `<a href="${escapeHtml(attribution.href)}">${escapeHtml(attribution.title)}</a>` : escapeHtml(attribution.title);
  return `${title} · ${escapeHtml(attribution.author)} · ${licenseLink}`;
}

function coverTitle(publication, localeConfig, cover) {
  if (!cover || cover.showTitle === false) return null;
  return localeConfig.coverTitle ?? cover.title ?? localeConfig.title;
}

async function writeCover(publicationWorkDir, publication, locale, localeConfig, themeDestination, version) {
  ...
  if (cover.showMetadata !== false) {
    if (publication.author) metadata.push(`<div class="publication-cover-author">${escapeHtml(publication.author)}</div>`);
    const edition = [];
    if (version) edition.push(`${escapeHtml(labels.version)} ${escapeHtml(version)}`);
    if (publication.revision) edition.push(`${escapeHtml(labels.revision)} ${escapeHtml(publication.revision)}`);
    if (edition.length > 0) metadata.push(`<div class="publication-cover-edition">${edition.join(' · ')}</div>`);
    const license = licenseHtml(publication.license);
    if (license) metadata.push(`<div class="publication-cover-license">${license}</div>`);
    const lineage = [];
    const designedWith = linkHtml(publication.lineage?.designedWith);
    const poweredBy = linkHtml(publication.lineage?.poweredBy);
    if (designedWith) lineage.push(`${escapeHtml(labels.designedWith)} ${designedWith}`);
    if (poweredBy) lineage.push(`${escapeHtml(labels.poweredBy)} ${poweredBy}`);
    if (lineage.length > 0) metadata.push(`<div class="publication-cover-lineage">${lineage.join(' · ')}</div>`);
  }
  ...
  return {
    entry: {rel: 'cover', path: 'publication-cover.html', output: 'cover.html', theme: themeDestination},
    cover: {src: cover.image, name: title ?? localeConfig.title},
  };
}

async function preparePublication(publicationName, publication, locale, localeConfig, version) {
